...
def show_product_detail(asin):
    ...
    with col_img:
        ...
        st.image(image_url if pd.notna(image_url) and image_url else PLACEHOLDER_IMAGE, width="stretch")
        st.markdown(f"[🔗 فتح المنتج على أمازون]({product_row.get('Product URL')})")
    ...
    with col_info:
        ...
        if not product_offers.empty:
            ...
            st.markdown(f"أرخص عرض حاليًا من: **{seller_name if pd.notna(seller_name) else '—'}**")

        with st.expander("📄 عرض الوصف الكامل", expanded=False):
            description = product_row.get("Description")
            st.write(description if pd.notna(description) and description else "لا يوجد وصف متاح لهذا المنتج.")
    ...
    for chunk in rows:
        ...
        for col, (_, product) in zip(cols, chunk.iterrows()):
            with col:
                ...
                title = title if pd.notna(title) and title else "بدون عنوان"
                short_title = title if len(str(title)) <= 60 else str(title)[:57] + "..."

                st.markdown('<div class="product-card">', unsafe_allow_html=True)
                st.image(image_url if pd.notna(image_url) and image_url else PLACEHOLDER_IMAGE)
